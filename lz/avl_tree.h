...
using namespace std;
	...
	template<typename Bst, typename Vistors = BstVisitors>
	struct BstImplement
	{
...
//below implement bst concept
	...
	template<typename _Node, typename _KeyCompare = std::less<typename _Node::KeyType> >
	struct AvlTree: public BinerySearchTree<_Node, _KeyCompare>
	{
		using Base = BinerySearchTree<_Node, _KeyCompare>;
	public:
		using Base::key;
		using typename Base::NodeDescriptor;

		using HeightType = typename _Node::HeightType;
//		using NodeDescriptor = typename Base::NodeDescriptor;
	...
	template<typename AvlTree, typename Visitors = BstVisitors>
...
//		using ND = _Node*;
		using VertexDescriptor = _Node*;
	public:
		using SizeType = typename _Node::SizeType;
		SizeType& size(VertexDescriptor u) { return u->size; }
		const SizeType& size(VertexDescriptor u) const { return u->size; }
	};
...
//	typename SizedBst::NodeDescriptor SizedBstAt(const SizedBst &bst, typename SizedBst::SizeType i)
//	{
//		typename SizedBst::NodeDescriptor u = bst.root();
//		for(;;)
//		{
...
//			{
//				NodeDescriptor res = BstImplement::erase(x);
...
//		}
//
//		static void erase(AvlTree &bst, const KeyType &key)
//		{
//			erase(bst, BstImplement::find(bst, key));
//		}
//
//	};
//
