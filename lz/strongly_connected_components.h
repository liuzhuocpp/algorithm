...
//	using TimeType = typename std::decay<typename MapTraits<DiscoverTimeMap>::ValueType>::type;
	...
	struct Impl: public DepthFirstSearchParams
	{
		...
		using VertexIndexMap = ChooseParamType<ParamVertexIndexMap, DefaultVertexIndexMap>;

		using ParamDiscoverTimeMap = typename std::result_of< decltype(&Params::discoverTimeMap)(Params)>::type;
		using DefaultDiscoverTimeMap = ComposeMap<VertexIndexMap, IteratorMap<size_t*> >;
		using DiscoverTimeMap = ChooseParamType<ParamDiscoverTimeMap, DefaultDiscoverTimeMap>;

		using ParamRootMap = typename std::result_of< decltype(&Params::rootMap)(Params)>::type;
		using DefaultRootMap = ComposeMap<VertexIndexMap, IteratorMap<V*> >;
		using RootMap = ChooseParamType<ParamRootMap, DefaultRootMap>;
